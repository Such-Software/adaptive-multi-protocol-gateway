...
# Daemon config formats (torrc, i2pd tunnels.conf, systemd units) are
# line-oriented and have NO quoting or escape mechanism, so a single newline in
# an interpolated value injects an arbitrary directive: a `tunnel_name` of
# "web\nhost = attacker.example" silently repoints the tunnel. Because there is
# nothing to escape WITH, these validators reject rather than sanitize.
#
# `_validate_site_id` in onboarding.py covers the `init` path only. A config
# loaded from disk never passes through it, so validation also has to happen
# here, at the point of interpolation.
_CONFIG_TOKEN_ALLOWED = frozenset(
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-."
)


def _validate_config_token(value: str, field: str) -> str:
    """Return `value` if it is safe to interpolate into a daemon config line."""
    if not value:
        raise ValueError(f"{field} must not be empty")
    if len(value) > 128:
        raise ValueError(f"{field} must be 128 characters or fewer")
    rejected = sorted({char for char in value if char not in _CONFIG_TOKEN_ALLOWED})
    if rejected:
        raise ValueError(
            f"{field} may only contain letters, numbers, and _-. characters; "
            f"rejected {''.join(rejected)!r}"
        )
    return value


def _validate_config_path(path: Path, field: str) -> Path:
    """Return `path` if it cannot break out of its config line."""
    text = str(path)
...
def _daemon_config_content(
    ...
    state_dir = _validate_config_path(
        _state_dir(config, provider, site, protocol), "state directory"
    )
    if protocol.name == "tor":
...
"""
    if protocol.name == "i2p":
        ...
        inport = int(protocol.options.get("inport", 80))
        key_file = _validate_config_token(
            protocol.options.get("keys_file", f"{site.id}-web.dat"), "keys_file"
        )
        tunnel_name = _validate_config_token(
            protocol.options.get("tunnel_name", f"{site.id}-web"), "tunnel_name"
        )
        return f"""# Generated by AMPG. Review before installing.
[{tunnel_name}]
type = http
...
def _supervisor_content(
    ...
    *,
    ...
    if provider.name == "linux-systemd":
        # A unit with no User= runs as whoever installs it, which is normally
        # root. Emit User=/Group= when the operator has configured a service
        # account, and say so loudly in the generated file otherwise, since the
        # whole point of the plan/approve flow is that a human reads this.
        service_user = protocol.options.get("service_user")
        if service_user:
            service_user = _validate_config_token(service_user, "service_user")
            service_group = _validate_config_token(
                protocol.options.get("service_group", service_user), "service_group"
            )
            account_lines = f"User={service_user}\nGroup={service_group}\n"
        else:
            account_lines = (
                "# WARNING: no service_user configured, so this unit runs as the\n"
                "# user that installs it (normally root). Set `service_user` in the\n"
                "# protocol options, or add User=/Group= below before installing.\n"
            )
        return f"""# Generated by AMPG. Review before installing.
[Unit]
...
[Service]
Type=simple
{account_lines}WorkingDirectory={_state_dir(config, provider, site, protocol)}
ExecStart=/bin/sh -lc '{_shell_single_quote(command)}'
Restart=on-failure
RestartSec=5
# Baseline hardening. Kept to directives that cannot break a daemon whose
# state directory may live anywhere (so no ProtectSystem/ProtectHome here);
# tighten per deployment once the state path is known.
...
"""
    ...
    state_dir = _state_dir(config, provider, site, protocol)
    data_dir_setup = f"\nmkdir -p {state_dir / 'data'}" if protocol.name == "i2p" else ""
    return f"""#!{shell}
# Generated by AMPG. Review before installing.
set -eu
mkdir -p {state_dir}{data_dir_setup}
{command}
"""
